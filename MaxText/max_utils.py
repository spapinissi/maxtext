...
def _retrieve_jax_init_info(raw_keys):
    ...
    local_jax_init_info_file = (
        epath.Path(raw_keys["local_checkpoint_directory"]) / JAX_INIT_INFO_FILE
    )
...
def setup_decode_state(model, config, rng, mesh, checkpoint_manager):
    ...
    if not config.load_parameters_path:
        ...
        unboxed_abstract_state, state_mesh_annotations, _ = get_abstract_state(
            model, None, config, rng, mesh, False
        )
        with nn_partitioning.axis_rules(config.logical_axis_rules):
            params = checkpointing.load_params_from_path(
                config.load_parameters_path, unboxed_abstract_state.params
            )
...
def print_system_information():
    ...
    max_logging.log(
        f"System Information: Jax Backend: {jax.lib.xla_bridge.get_backend().platform_version}"
    )
