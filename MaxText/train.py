...
def record_scalar_metrics(
    metrics, step_time_delta, per_device_tflops, lr, per_device_tokens
):
    ...
    metrics["scalar"].update(
        {
            "perf/per_device_tokens_per_sec": per_device_tokens
            / step_time_delta.total_seconds()
        }
    )
...
def save_checkpoint(
    ...
    if isinstance(checkpoint_manager, emergency_checkpoint_manager.CheckpointManager):
        return checkpoint_manager.save(
            step,
            args=orbax.checkpoint.args.PyTreeSave(
                item=state,
                save_args=save_args,
                ocdbt_target_data_file_size=_CHUNK_BYTE_SIZE,
            ),
        )
...
def loss_fn(model, config, data, dropout_rng, params, is_train=True):
    ...
    if config.num_experts > 1:
        ...
        total_moe_lb_loss = maxtext_utils.get_nested_value(
            intermediate_outputs, nested_key, 0.0
        )
    ...
    if config.gradient_accumulation_steps > 1:
        ...
        def accumulate_gradient(acc_grad_and_loss, data):
            ...
            (_, aux), cur_batch_gradient = grad_func(
                model, config, data, dropout_rng, state.params, is_train=True
            )
        ...
        def reshape_to_microbatch_accumulations(batch_arr):
            ...
            microbatch_shape = (
                microbatches,
                batch_arr.shape[0] // microbatches,
            ) + batch_arr.shape[1:]
        ...
        raw_grads = jax.tree_util.tree_map(
            lambda arr: arr / grad_and_loss["total_weights"], grad_and_loss["grad"]
        )
...
def setup_train_loop(config):
    ...
    if not config.using_pipeline_parallelism:
        ...
        maxtext_utils.assert_params_sufficiently_sharded(
            state.params, mesh, tolerance=0.02
        )
    ...
    for step in np.arange(start_step, config.steps):
        ...
        if (
            ...
            for eval_batch in eval_data_iterator:
                ...
                cumulative_eval_metrics["moe_lb_loss"] += float(
                    eval_metrics["scalar"]["evaluation/moe_lb_loss"]
                )
                max_logging.log(f"Completed eval step {eval_step_count}")
                eval_step_count += 1
            eval_loss = (
                cumulative_eval_metrics["total_loss"]
