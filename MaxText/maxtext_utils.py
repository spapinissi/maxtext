...
def get_functional_eval_with_signature(
...
def load_compiled(config, partial_train, state):
    ...
    shaped_input_kwargs = {}
    in_tree, out_tree = get_train_input_output_trees(
        partial_train, shaped_input_args, shaped_input_kwargs
    )
...
def calculate_gemma2_tflops_training_per_device(
...
def calculate_prefill_tflops_per_device(
    num_model_parameters, prefill_length, config, log=True
):
    """Calculate training TFLOP"""
    learnable_weight_tflops = (
        2 * num_model_parameters * prefill_length / jax.device_count() / 1e12
    )
    ...
    causal_attention_tflops = (
        noncasual_attention_flops / 2
    )  # due to causality in attention
...
def apply_gradient_clipping(raw_grads, state, clipping_threshold):
    ...
    if OVERWRITE_WITH_GRADIENT in raw_grads:
        ...
        grads[OVERWRITE_WITH_GRADIENT] = (
            fp8_stats  # pytype: disable=unsupported-operands
        )
        raw_grads[OVERWRITE_WITH_GRADIENT] = (
            fp8_stats  # pytype: disable=unsupported-operands
        )
    else:
        grads, _ = gradient_clip_transformation.update(raw_grads, state, None)
